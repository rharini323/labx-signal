...
type Quest = { id:string; title:string; description:string; reward:number; category:string };
type CoinEntry = { id:string; label:string; amount:number; date:string };
type Detail = { kind:'project'|'startup'|'mentor'|'opportunity'|'profile'; data:any } | null;
...
const quests: Quest[] = [
 {id:'profile',title:'Complete your LabX profile',description:'Open your profile and review your LabX identity.',reward:25,category:'STARTER'},
 {id:'follow-builder',title:'Follow a builder',description:'Follow at least one person from the LabX community.',reward:10,category:'COMMUNITY'},
 {id:'explore-project',title:'Explore a project',description:'Open a project profile and learn what the team is building.',reward:20,category:'DISCOVER'},
 {id:'mentor-profile',title:'Meet the mentors',description:'Open a mentor profile and explore their expertise.',reward:20,category:'CONNECT'},
 {id:'roadmap-stage',title:'Complete a roadmap stage',description:'Finish any roadmap stage and keep your momentum moving.',reward:50,category:'ROADMAP'},
 {id:'daily',title:'Claim your daily reward',description:"Return to LabX and claim today's daily coins.",reward:25,category:'DAILY'}
];
...
function LabXModal({title,eyebrow,onClose,children}:{title:string;eyebrow?:string;onClose:()=>void;children:ReactNode}){
 return <div style={{position:'fixed',inset:0,zIndex:1000,background:'rgba(3,7,14,.78)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}} onClick={onClose}>
  <div style={{width:'min(860px,100%)',maxHeight:'88vh',overflowY:'auto',background:'#0a0f18',border:'1px solid rgba(255,255,255,.10)',boxShadow:'0 30px 90px rgba(0,0,0,.45)',padding:'28px'}} onClick={e=>e.stopPropagation()}>
   <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:20,marginBottom:24}}>
    <div>{eyebrow&&<small style={{letterSpacing:'.16em',opacity:.65}}>{eyebrow}</small>}<h2 style={{margin:'8px 0 0',fontSize:'clamp(26px,4vw,42px)'}}>{title}</h2></div>
...
function DetailContent({detail,onClose,onFollow,following,currentUserName,onOpenProfile}:{detail:Exclude<Detail,null>;onClose:()=>void;onFollow:(name:string)=>void;following:string[];currentUserName:string;onOpenProfile:(name:string)=>void}){
 const {kind,data}=detail;
 if(kind==='profile'){
  const profile=data as Profile;
  const isSelf=profile.name===currentUserName;
  const isFollowing=following.includes(profile.name);
  return <LabXModal title={profile.name} eyebrow={isSelf?'MY PROFILE':'LABX PROFILE'} onClose={onClose}>
   <article className="project-row"><div className="initials">{profile.initials}</div><div><small>{profile.role} / {profile.expertise}</small><p>{profile.bio}</p><div className="tags"><span>{profile.location}</span><span>{profile.status}</span></div></div><div className="row-meta"><span>{profile.coins} COINS</span>{!isSelf&&<button className="ghost" onClick={()=>onFollow(profile.name)}>{isFollowing?'FOLLOWING':'FOLLOW'}</button>}</div></article>
   <article className="project-row"><span className="index">STATS</span><div><small>COMMUNITY ACTIVITY</small><p>{profile.projects} PROJECTS</p><div className="tags"><button className="ghost" onClick={()=>{const target=people.find(p=>p.name!==profile.name);if(target) onOpenProfile(target.name)}}>{profile.followers} FOLLOWERS</button><button className="ghost" onClick={()=>{const target=people.find(p=>p.name!==profile.name && p.name!==currentUserName);if(target)onOpenProfile(target.name)}}>{profile.following} FOLLOWING</button></div><div className="tags">{profile.achievements.map((a:string)=><span key={a}>{a}</span>)}</div></div></article>
  </LabXModal>;
 }
 if(kind==='project'){const p=data as Project;return <LabXModal title={p.name} eyebrow={`${p.category} / ${p.status}`} onClose={onClose}><article className="project-row"><div><small>PROJECT</small><p>{p.description}</p><div className="tags">{p.skills.map((x:string)=><span key={x}>{x}</span>)}</div></div><div className="row-meta"><span>{p.members} MEMBERS</span></div></article><article className="project-row"><span className="index">NEXT</span><div><small>PROJECT PARTICIPATION</small><p>Review the project, understand the skills needed and connect with the team.</p></div></article></LabXModal>}
 if(kind==='startup'){const x=data as Startup;return <LabXModal title={x.name} eyebrow={`${x.sector} / ${x.stage}`} onClose={onClose}><article className="project-row"><div><small>STARTUP</small><p>{x.description}</p><div className="tags"><span>{x.location}</span><span>{x.size} PEOPLE</span></div></div></article><article className="project-row"><span className="index">OPEN</span><div><small>ECOSYSTEM</small><p>Explore this startup, its stage and potential opportunities to contribute.</p></div></article></LabXModal>}
 if(kind==='mentor'){const m=data as Mentor;return <LabXModal title={m.name} eyebrow={`${m.expertise} / MENTOR`} onClose={onClose}><article className="project-row"><div className="initials">{m.initials}</div><div><small>{m.role}</small><p>LabX mentor specializing in {m.expertise}.</p><div className="tags"><span>{m.availability}</span><span>MENTOR NETWORK</span></div></div><div className="row-meta"><button className="ghost" onClick={onClose}>CONNECT</button></div></article></LabXModal>}
 const o=data as Opportunity;return <LabXModal title={o.title} eyebrow={`${o.type} / ${o.org}`} onClose={onClose}><article className="project-row"><div><small>{o.location} / {o.deadline}</small><p>Explore this opportunity and review its requirements before applying.</p><div className="tags">{o.tags.map((x:string)=><span key={x}>{x}</span>)}</div></div><div className="row-meta"><button className="ghost" onClick={onClose}>VIEW</button></div></article></LabXModal>;
}
...
function App(){
 ...
 const [utilityView,setUtilityView]=useState<'quests'|'coins'|null>(null);
 const [claimedQuests,setClaimedQuests]=useState<string[]>(()=>{try{return JSON.parse(localStorage.getItem('labx_claimed_quests')||'[]')}catch{return []}});
 const [coinHistory,setCoinHistory]=useState<CoinEntry[]>(()=>{try{return JSON.parse(localStorage.getItem('labx_coin_history')||'[]')}catch{return []}});
 const [dailyClaimed,setDailyClaimed]=useState(()=>localStorage.getItem('labx_daily_claim')===new Date().toISOString().slice(0,10));
 ...
 const awardCoins=(amount:number,label='LabX activity')=>{
  setCoinBalance(current=>current+amount);
  setCoinHistory(current=>[{id:`${Date.now()}-${amount}`,label,amount,date:new Date().toLocaleDateString()},...current].slice(0,20));
 };
 useEffect(()=>{localStorage.setItem('labx_claimed_quests',JSON.stringify(claimedQuests))},[claimedQuests]);
 useEffect(()=>{localStorage.setItem('labx_coin_history',JSON.stringify(coinHistory))},[coinHistory]);
 const claimQuest=(quest:Quest)=>{if(claimedQuests.includes(quest.id))return;setClaimedQuests(current=>[...current,quest.id]);awardCoins(quest.reward,quest.title);};
 const claimDaily=()=>{const today=new Date().toISOString().slice(0,10);if(dailyClaimed)return;setDailyClaimed(true);localStorage.setItem('labx_daily_claim',today);awardCoins(25,'Daily coin claim');};
 const toggleFollow=(name:string)=>{if(followedPeople.includes(name)){setFollowedPeople(current=>current.filter(person=>person!==name));return;}setFollowedPeople(current=>[...current,name]);awardCoins(10,`Followed ${name}`)};
 const toggleStage=(number:string)=>{if(completedStages.includes(number)){setCompletedStages(current=>current.filter(item=>item!==number));return;}setCompletedStages(current=>[...current,number]);awardCoins(50,`Completed roadmap stage ${number}`)};
 ...
 useEffect(()=>{const sync=()=>{const id=window.location.hash.replace(/^#/,'');if(['discover','projects','startups','mentors','opportunities','people','followers','roadmap','leaderboard','about'].includes(id))setActive(id||'discover')};sync();window.addEventListener('hashchange',sync);return()=>window.removeEventListener('hashchange',sync)},[]);
 const go=(id:string)=>{setActive(id);window.history.replaceState(null,'',`#${id}`)};
 const openProfile=(name:string)=>{const profile=getProfile(name);if(profile)setDetail({kind:'profile',data:profile})};
 ...
 return <div className="app"><SignalField/>
  ...
  <main>
   ...
   {active==='about'&&<section id="about" className="section about"><div className="section-intro-grid"><SectionTitle eyebrow="09 / ABOUT LABX" title="An ecosystem designed around momentum.">LabX brings together people with ideas, builders, startups, mentors, and opportunities so progress doesn&apos;t stop at the idea stage.</SectionTitle><ThreeSectionVisual variant="about"/></div><div className="about-grid"><div className="about-statement">IDEAS ARE<br/><em>ONLY THE START.</em></div><div className="about-copy"><p>LabX connects the pieces that usually live in separate places.</p><div className="about-points"><span><b>01</b> DISCOVER</span><span><b>02</b> CONNECT</span><span><b>03</b> BUILD</span><span><b>04</b> VALIDATE</span><span><b>05</b> LAUNCH</span><span><b>06</b> GROW</span></div></div></div></section>}
   {active==='discover'&&<section className="how"><div className="how-top"><div><div className="how-label">HOW IT WORKS</div><h2>From signal to momentum.</h2><p>Every stage has a purpose. Move through the ecosystem without losing the thread.</p></div><ThreeSectionVisual variant="journey"/></div><div className="journey">{['DISCOVER','CONNECT','BUILD','VALIDATE','LAUNCH','GROW'].map((x,i)=><div className="journey-step" key={x}><span>{String(i+1).padStart(2,'0')}</span><b>{x}</b></div>)}</div></section>}
  </main>
  {detail&&<DetailContent detail={detail} onClose={()=>setDetail(null)} onFollow={toggleFollow} following={followedPeople} currentUserName={currentUserName} onOpenProfile={openProfile}/>}
  {utilityView==='quests'&&<LabXModal title="LabX Quests" eyebrow="REWARDS / MOMENTUM" onClose={()=>setUtilityView(null)}>
   <div className="project-list">{quests.map(q=>{const claimed=claimedQuests.includes(q.id);return <article className="project-row" key={q.id}><span className="index">+{q.reward}</span><div><small>{q.category}</small><h3>{q.title}</h3><p>{q.description}</p></div><div className="row-meta"><span>{claimed?'CLAIMED':'REWARD'}</span><button className="ghost" disabled={claimed} onClick={()=>claimQuest(q)}>{claimed?'CLAIMED':`CLAIM +${q.reward}`}</button></div></article>})}</div>
  </LabXModal>}
  {utilityView==='coins'&&<LabXModal title={`${coinBalance} LABX COINS`} eyebrow="REWARDS WALLET" onClose={()=>setUtilityView(null)}>
   <article className="project-row"><div><small>DAILY REWARD</small><h3>{dailyClaimed?"Today's reward claimed":'Claim your daily coins'}</h3><p>{dailyClaimed?'Come back tomorrow for the next daily reward.':'Claim +25 coins once per day.'}</p></div><div className="row-meta"><button className="ghost" disabled={dailyClaimed} onClick={claimDaily}>{dailyClaimed?'CLAIMED':'CLAIM +25'}</button></div></article>
   <div className="result-line">RECENT COIN ACTIVITY</div>
   <div className="project-list">{coinHistory.length?coinHistory.map(entry=><article className="project-row" key={entry.id}><span className="index">+{entry.amount}</span><div><small>{entry.date}</small><h3>{entry.label}</h3></div><div className="row-meta"><span>COINS</span></div></article>):<div className="result-line">COMPLETE QUESTS, FOLLOW PEOPLE OR FINISH ROADMAP STAGES TO START EARNING.</div>}</div>
  </LabXModal>}
